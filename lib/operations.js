  ...
  if (query.hostname) {
    ...
    for (var i = 0; i<server.tags.length; i++) {
      ...
      if (current_tag.hostnames && current_tag.hostnames > 0) {
        database[config.tag_collection].update(current_tag);
      } else {
    ...
    for (var j = 0; j < query.tags.length; j++) {
      var tag = database[config.tag_collection].find({'tag': query.tags[j]});
      for (var k = 0; k < tag[0].hostnames.length; k++) {
        var db_host = database[config.server_collection].findOne({'hostname': tag[0].hostnames[k]});
        if (verbose) {
          console.log(db_host);
        }
        db_host.tags = removeTagFromHost(db_host, tag[0].tag);
        if (verbose) {
          console.log(db_host);
        }
        database[config.server_collection].update(db_host);
      }
